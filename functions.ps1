...
function install_git_dependencies
{
  ForEach($g_dep in $Env:GIT_DEPENDENCIES.split(' '))
  {
    ...
    echo cmake .. -G "Visual Studio 14 2015 Win64" -DCMAKE_INSTALL_PREFIX="${Env:CMAKE_INSTALL_PREFIX}" -DPYTHON_BINDING=OFF -DMINGW_GFORTRAN="$env:MINGW_GFORTRAN"  -DCMAKE_BUILD_TYPE=Release
